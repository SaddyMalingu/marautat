...
async function loadBlogs(){const r=await fetch('/admin/ai-jobs/blogs',{headers:{'x-admin-key':AK}});const d=await r.json();if(d.blogs){const l=document.getElementById('allBlogsList');l.innerHTML=d.blogs.map(b=>'<div style="background:rgba(255,255,255,.05);padding:1rem;border-radius:8px;margin:.5rem 0"><a href="'+b.url+'" target="_blank" style="color:#ff8a00;font-weight:bold">'+b.title+'</a>'+(b.hasImage?' 🖼️':'')+' <span style="color:#b8c7d6;font-size:.8rem">('+b.slug+')</span></div>').join('')+'<p style="color:#b8c7d6">Total: '+d.blogs.length+' blog posts</p'}}
